# --- IMPORTANT: Ensure these imports are at the VERY TOP of your file ---
...
# --- ROAD NETWORK CACHING (Add this above your main app logic) ---
@st.cache_resource
def get_road_network():
    # Mumbai center coordinates - Cached to prevent reloading
    return ox.graph_from_point((19.0760, 72.8777), dist=8000, network_type='drive')

# --- MAIN PAGE LOGIC ---
# (Make sure your 'if page == ...' block starts before this)

if page == "Home":
    st.title("Home Page")
    st.write("Welcome to the Smart Waste Dashboard")

elif page == "Route Optimization":
    st.title("🚛 AI Multi-Fleet Dispatcher & Navigation")
    
    # 1. Define Garages and Disposal Site
    GARAGES = {
    ...
    # --- THE MENTOR'S SECRET: TIME TRAVEL SLIDER ---
    # This allows you to show bins at 75% by picking a time they were actually full
    st.sidebar.subheader("📅 Simulation Time")
    # Clean the timestamp to ensure it's a datetime object
    df['timestamp'] = pd.to_datetime(df['timestamp'])
    unique_times = sorted(df['timestamp'].unique())
    
    # We pick a time where bins are historically full (80% into the dataset)
    ...
    # Filter bins for the specific mission
    ...
    try:
        ...
        for _, row in df_snapshot.iterrows():
            ...
            if is_full and is_mine: color = 'red' # My Task
            elif is_full and not is_mine: color = 'orange' # Someone else's task
        ...
        if not my_bins.empty:
            # Sort bins by fill percent (Priority) and take top 8 for QR stability
        ...
        for name, coords in GARAGES.items():
            folium.Marker(coords, popup=name, 
                          icon=folium.Icon(color='blue' if name == selected_truck else 'gray', icon='truck', prefix='fa')).add_to(m)
        ...
        st.error(f"Mapping System Initializing: {e}")
